...
def word_counter( string, dictionary )
  array_of_count = dictionary.map {
    | e | string.downcase.scan(e).length
  }
  return dictionary.zip( array_of_count ).to_h.select{ 
    | key, value | value != 0
  }
end
...
#puts word_counter(open("shakespeare.txt").read, open())
